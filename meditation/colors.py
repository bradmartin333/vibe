...
def hue_shift(time: float, speed: float = 0.15, saturation: float = 0.5,
              brightness: float = 0.6, alpha: int = 255) -> rl.Color:
    ...
    match i % 6:
        ...
        case _: r, g, b = brightness, p, q
    return rl.Color(_clamp(int(r * 255)), _clamp(int(g * 255)),
                    _clamp(int(b * 255)), _clamp(alpha))
...
def trippy(time: float, layer: int = 0, alpha: int = 255) -> rl.Color:
    ...
    return hue_shift(time + offset, speed=0.08, saturation=0.35,
                     brightness=0.55, alpha=alpha)
